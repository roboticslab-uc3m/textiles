/* kinectGrabber
 * ----------------------
 * Nice program to display a Kinect RGB and depth stream. Saving frames to files
 * is supported.
 *
 * This is heavily based on the glview.c example from the OpenKinect Project. 
 * Therefore, we are keeping the original copyright notice below this comment.
 *
...
void saveImageToFile(uint8_t* image, int width, int height, const char* filepath)
{
    //-- Open File
    int i;
