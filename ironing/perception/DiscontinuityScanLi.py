...
if __name__ == '__main__':
    for i in range(1, 4): # For each sample image
        # Load source images
        ...
        ax[0].imshow(np.where(roi1 <= 1, roi1, 0), cmap=plt.cm.viridis)
        ax[1].imshow(np.where(roi2 <= 1, roi2, 0), cmap=plt.cm.viridis)
        ax[2].imshow(np.where(roi <= 1, roi, 0) , cmap=plt.cm.viridis)
        plt.show()
