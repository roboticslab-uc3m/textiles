...
#--------------------------------------------------------------
rgbOptions = yarp.Property()
rgbOptions.put('device', 'remote_grabber')
rgbOptions.put('remote', '/OpenNI2/imageFrame:o')
rgbOptions.put('local', '/rgb:i')
rgbDevice = yarp.PolyDriver(rgbOptions)
if not rgbDevice.isValid():
    print 'no rgb'
    quit()
rgbGrabber = rgbDevice.viewFrameGrabberImage()

p = yarp.BufferedPortImageMono16()
p.open('/depth:i');
yarp.Network.connect('/OpenNI2/depthFrame:o','/depth:i')
...
yarp_image.setExternal(frame, frame.shape[1], frame.shape[0])

# Read yarp image from stream
rgbGrabber.getImage(yarp_image)
frame = cv2.cvtColor(frame, cv2.COLOR_RGB2BGR)
plt.figure()
plt.imshow(frame)
plt.show()

print '\t\tGetting dFrame...'
# Create numpy array to receive the image and the YARP image wrapped around it
dFrame = np.zeros((480, 640), dtype=np.uint16)
yarp_dImage = yarp.ImageMono16()
yarp_dImage.resize(640, 480)
#yarp_dImage.setExternal(dFrame, frame.shape[1], frame.shape[0])
#yarp_dImage.setExternal(dFrame.__array_interface__['data'][0], dFrame.shape[1], dFrame.shape[0])
# Read yarp image from stream
p.read(yarp_dImage)
print '\t\Tested Got dFrame.'

#print yarp_dImage.getRawImage()
#pA = ctypes.cast( yarp_dImage.getRawImage(), ctypes.POINTER( ctypes. ) )
#print pA[0]
#if yarp_dImage.getRawImage().__long__() <> dFrame.__array_interface__['data'][0]:
#    print "read() reallocated my yarp_image!"
for i in range(yarp_dImage.width()):
    for j in range(yarp_dImage.height()):
        print yarp_dImage.getPixel(i,j)
        dFrame[j,i] = yarp_dImage.getPixel(i,j)
#        var = ctypes.cast( yarp_dImage.pixel(i,j).__long__(), ctypes.c_uint16 )
#        print var[i]
#        dFrame[j,i] = yarp_dImage.getPixel(i,j)
#var = ctypes.cast( yarp_dImage.pixel(0,0).__long__(), ctypes.POINTER(ctypes.c_uint16) )
#for i in range(yarp_dImage.width()):
#    print var[i]
#dFrame = cv2.cvtColor(dFrame, cv2.COLOR_RGB2GRAY)
plt.figure()
plt.imshow(dFrame)
plt.show()
